...
class ModelMetaclass(type):
    ...
    def __getattr__(
        cls, name: str
    ) -> Union[field.Field, relationship.Relationship, index.Index]:
...
class ModelApi(metaclass=ModelMetaclass):
  ...
  """
    ...
    @classmethod
    def find(
        cls, transaction: Optional[spanner_transaction.Transaction] = None, **keys: Any
    ...
    def _results_to_models(
        cls, results: Iterable[Iterable[Any]]
    ) -> List["ModelObject"]:
...
class Model(ModelApi):
    ...
    def reload(
        self, transaction: spanner_transaction.Transaction = None
    ) -> Optional["Model"]:
