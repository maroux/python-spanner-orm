...
class ColumnsEqualCondition(Condition):
    ...
    def _validate(self, model_class: Type[Any]) -> None:
        ...
        if origin.field_type != dest.field_type or origin.nullable != dest.nullable:
            raise error.ValidationError(
                "Types of {} and {} do not match".format(origin.name, dest.name)
            )
...
class ForceIndexCondition(Condition):
    ...
    def _validate(self, model_class: Type[Any]) -> None:
        for idx in model_class.indexes.values():
            ...
            raise error.ValidationError(
                "{} is not an index on {}".format(self.name, model_class.table)
            )
...
class IncludesCondition(Condition):
    ...
    def single(self) -> bool:
        if not self.relation:
            raise error.SpannerError("Condition must be bound before single is called")
    ...
    def _validate(self, model_class: Type[Any]) -> None:
        if self.name not in model_class.relations:
            raise error.ValidationError(
                "{} is not a relation on {}".format(self.name, model_class.table)
            )
        if self.relation and self.relation != model_class.relations[self.name]:
            raise error.ValidationError(
                "{} does not belong to {}".format(self.relation.name, model_class.table)
            )
    ...
    def _sql(self) -> str:
        ...
        for (column, order_type) in self.orderings:
            if isinstance(column, field.Field):
                column = column.name
            orders.append(
                "{alias}.{column} {order_type}".format(
    ...
    def _validate(self, model_class: Type[Any]) -> None:
        for (column, _) in self.orderings:
            ...
            if column not in model_class.fields:
                raise error.ValidationError(
                    "{} is not a column on {}".format(column, model_class.table)
                )
...
class ComparisonCondition(Condition):
    ...
    def __init__(
        self, operator: str, field_or_name: Union[field.Field, str], value: Any
    ):
        super().__init__()
    ...
    def _validate(self, model_class: Type[Any]) -> None:
        ...
        if self.field and self.field != model_class.fields[self.column]:
            raise error.ValidationError(
                "{} does not belong to {}".format(self.column, model_class.table)
            )
        if self.value is None:
            raise error.ValidationError(
                "{} does not support NULL".format(self.__name__)
            )
...
def includes(
    relation: Union[relationship.Relationship, str], conditions: List[Condition] = None
) -> IncludesCondition:
...
def in_list(
    column: Union[field.Field, str], values: Iterable[Any]
...
def not_in_list(
    column: Union[field.Field, str], values: List[Any]
) -> ListComparisonCondition:
