...
class AdminTest(unittest.TestCase):
    ...
    def test_metadata(self, tables, columns, index_columns, indexes):
        ...
        for row in model.columns:
            self.assertEqual(meta.fields[row].field_type, model.fields[row].field_type)
            self.assertEqual(meta.fields[row].nullable, model.fields[row].nullable)
    ...
    def test_interleaved(self, tables, columns, index_columns, indexes):
        ...
        tables.return_value = self.make_test_tables(
            model, parent_table=parent_model.table
        ) + self.make_test_tables(parent_model)
        columns.return_value = self.make_test_columns(model) + self.make_test_columns(
            parent_model
        )
        index_columns.return_value = self.make_test_index_columns(
            model
        ) + self.make_test_index_columns(parent_model)
        indexes.return_value = self.make_test_index(model) + self.make_test_index(
            parent_model
        )
    ...
    def test_model_creation_ddl(self):
        ...
        self.assertCountEqual(
            models.IndexTestModel.meta.indexes.keys(),
            ["PRIMARY_KEY", "value_idx", "value_idx2"],
        )
    ...
    def test_model_creation_ddl2(self):
        ...
        self.assertEqual(ddl, expected_ddl)
        self.assertCountEqual(
            models.FieldCustomNameTestModel.meta.indexes.keys(), ["PRIMARY_KEY"]
        )
